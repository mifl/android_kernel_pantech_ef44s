	...
	struct tcbd_device *_device, u32 _freq_khz, s32 _bw_khz)
{
	s32 ret = 0;
	u64 tick;
	u32 buff_list[] = {TCBD_BUFFER_A_SIZE, 0, 0, 0};
	...
	ret |= tcbd_init_buffer_region(buff_list, _device);
	ret |= tcbd_enable_buffer(buff_list, _device);
				...
				struct tcbd_service *_service, s32 _flag)
{
	...
	u8 en_cmd_fifo = 0;
	u32 buff_list[] = {TCBD_MAX_FIFO_SIZE, 0, 0, 0};

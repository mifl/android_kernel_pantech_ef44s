...
s32 tcbd_enable_buffer(u32 *_list_buff, struct tcbd_device *_device)
{
	s32 i, ret = 0;
...
s32 tcbd_init_buffer_region(u32 *_list_buff, struct tcbd_device *_device)
{
	s32 i, ret = 0;
	...
	u32 addr_set_buff[TCBD_NUM_BUFFER<<1] = {0, };

	addr_end = PHY_BASE_ADDR + 0xA000;
	for (i = 0; i < TCBD_NUM_BUFFER; i += 2) {
		addr_set_buff[i] = addr_end>>2;
		addr_set_buff[i+1] = (addr_end+_list_buff[i>>1]-1)>>2;
		addr_end += _list_buff[i>>1];
	}
	...
	for (i = 0; i < TCBD_NUM_BUFFER; i++) {
		...
		tcbd_debug(DEBUG_DRV_COMP, "reg:%02x, %c buffer size: %d, "
					...
					addr_set_buff[(i<<1)+0]<<2,
					addr_set_buff[(i<<1)+1]<<2);
...
#if defined(__DEBUG_TCBD__)
	for (i = 0; i < TCBD_NUM_BUFFER && _list_buff[i]; i++) {

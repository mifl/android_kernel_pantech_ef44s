...
static inline s32 tcpal_i2c_reg_read_burst(u8 _reg_addr, u8 *_data, s32 _size)
{
  ...
  //TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], size[%d], num_chunk[%d], remain[%d], \n", __func__, _reg_addr, *_data, _size, num_chunk, remain);

  for (i = 0; i < num_chunk; i++)
  {
    tcc3170_i2c_read_len(I2C_ADDR, _reg_addr, &_data[i * MAX_I2C_BURST], MAX_I2C_BURST);

    //TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], idx[%d], size[%d]\n", __func__, _reg_addr, (unsigned int)_data[i * MAX_I2C_BURST], i * MAX_I2C_BURST, MAX_I2C_BURST);
  }
  ...
  if (remain)
  {
    ...
    //TDMB_MSG_TCC_BB("[%s] addr[0x%x], data[0x%x], idx[%d], size[%d]\n", __func__, _reg_addr, (unsigned int)_data[num_chunk * MAX_I2C_BURST], num_chunk * MAX_I2C_BURST, remain);
  }
